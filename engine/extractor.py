...
def extract_pages_text_from_pdf(file_bytes: bytes) -> List[Dict[str, Any]]:
    """Extract per-page embedded text for digital PDFs."""
...
def extract_all(file_bytes: bytes, enable_ocr_fallback: bool = True, ocr_dpi: int = 300) -> Dict[str, Any]:
    ...
    Returns a dict for the LLM:
      ...
      - pages: always populated page-level text
    ...
    final_pages = embedded_pages if embedded_pages else ocr_payload.get("pages", [])
