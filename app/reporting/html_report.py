...
def write_cli_style_report(
    *,
    ...
    # ── Snapshot sections (all modes) ──────────────────────────────────────
    visual_rows = _as_list(result_json.get("visual_validation"))
    visual_diff_error = str(result_json.get("visual_diff_error") or "").strip()

    # Detect benchmark from mode field OR from alignment_op values in visual rows
    is_benchmark = mode == "benchmark" or any(
        str(v.get("alignment_op") or "") in ("matched", "deleted", "inserted")
        for v in visual_rows if isinstance(v, dict)
    )

    # ── BENCHMARK: Three-panel Expected | Actual | Diff snapshot table ───────
    visual_comparison_table = ""
    if is_benchmark:
        bench_legend = (
            "<div style='font-size:12px;color:var(--muted);margin-bottom:10px;'>"
            "<b>Snapshot color codes (Actual / Diff panels):</b>&nbsp; "
            "<span style='color:#f87171;font-weight:600;'>&#9632; Red</span> = removed text &nbsp;"
            "<span style='color:#4ade80;font-weight:600;'>&#9632; Green</span> = added text &nbsp;"
            "<span style='color:#fb923c;font-weight:600;'>&#9632; Orange</span> = bold changed &nbsp;"
            "<span style='color:#facc15;font-weight:600;'>&#9632; Yellow</span> = font-size changed &nbsp;"
            "<span style='color:#60a5fa;font-weight:600;'>&#9632; Blue</span> = alignment shifted"
            "</div>"
        )
        if visual_diff_error:
            diff_error_banner = (
                f"<div style='background:rgba(220,38,38,0.12);border-left:4px solid #dc2626;"
                f"padding:10px 14px;border-radius:4px;margin-bottom:12px;font-size:13px;'>"
                f"<b style='color:#f87171;'>&#9888; Visual diff failed</b> — "
                f"three-panel snapshots could not be generated.<br/>"
                f"<span style='color:var(--muted);font-size:12px;'>Error: {_esc(visual_diff_error)}</span>"
                f"</div>"
            )
        else:
            diff_error_banner = ""
        ...
        for v in visual_rows:
            if not isinstance(v, dict):
                continue
            pg = v.get("page", "?")
            op = str(v.get("alignment_op") or "matched")
            sim = v.get("similarity")
            sim_text = f"{float(sim) * 100:.1f}%" if isinstance(sim, (int, float)) else "—"
            note = _shorten(str(v.get("note") or ""), 220)
            snap_path = str(v.get("snapshot_path") or "")
            snap_link = _snapshot_link(project_id, snap_path) if snap_path else "—"
            has_text = "Yes" if v.get("has_text_changes") else ("—" if op != "matched" else "No")
            severity_cls = "bad" if v.get("major") else ("warn" if v.get("warn") else "ok")
            op_label = {
                "matched": "MATCH", "deleted": "DELETED", "inserted": "INSERTED"
            }.get(op, op.upper())
            vc_rows_html.append(
                f"""<tr>
                  <td>{_esc(str(pg))}</td>
                  <td>{_chip(op_label, severity_cls)}</td>
                  <td>{_esc(sim_text)}</td>
                  <td>{_esc(has_text)}</td>
                  <td style="font-size:12px;color:var(--muted);">{_esc(note)}</td>
                  <td>{snap_link}</td>
                </tr>"""
            )

        empty_msg = (
            "<tr><td colspan='6' class='muted'>No three-panel snapshots — "
            + ("visual diff engine reported an error (see above)." if visual_diff_error else
               "poppler / pdf2image may not be installed, or PDFs could not be rendered.")
            + "</td></tr>"
        )

        visual_comparison_table = f"""
    <div class="card">
      <div class="section-title">
        <div>Visual Comparison — All Pages (Expected &#124; Actual &#124; Diff)</div>
        <div class="muted">Three-panel snapshots: left=baseline, centre=current, right=pixel diff overlay.</div>
      </div>
      {diff_error_banner}
      {bench_legend}
      {_render_table(
          ["Page", "Alignment", "Similarity", "Text Changes", "Note", "Snapshot"],
          "".join(vc_rows_html) or empty_msg
      )}
    </div>
    """

    # ── BASIC / SPECIFIC: single-panel page snapshot table ──────────────────
    page_snapshots_table = ""
    if not is_benchmark and visual_rows:
        basic_legend = (
            "<div style='font-size:12px;color:var(--muted);margin-bottom:10px;'>"
            "<b>Snapshot annotation color codes:</b>&nbsp; "
            "<span style='color:#f87171;font-weight:600;'>&#9632; Red</span> = critical &nbsp;"
            "<span style='color:#fb923c;font-weight:600;'>&#9632; Orange</span> = high &nbsp;"
            "<span style='color:#facc15;font-weight:600;'>&#9632; Yellow</span> = medium &nbsp;"
            "<span style='color:#60a5fa;font-weight:600;'>&#9632; Blue</span> = low"
            "</div>"
        )
        snap_rows_html = []
        for v in visual_rows:
            ...
            snap_path = str(v.get("snapshot_path") or "")
            snap_link = _snapshot_link(project_id, snap_path) if snap_path else "—"
            snap_rows_html.append(
                f"""<tr>
                  <td>{_esc(str(pg))}</td>
                  <td>{snap_link}</td>
                </tr>"""
            )
        page_snapshots_table = f"""
    <div class="card">
      <div class="section-title">
        <div>Page Snapshots</div>
        <div class="muted">Rendered pages with annotated findings highlighted by severity color.</div>
      </div>
      {basic_legend}
      {_render_table(
          ["Page", "Snapshot"],
          "".join(snap_rows_html) or "<tr><td colspan='2' class='muted'>No page snapshots available.</td></tr>"
      )}
    </div>
    """
